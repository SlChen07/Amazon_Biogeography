...
## a unified model for three drought new (Prediction) (Include DAG)
seg_target=12
...
Drought_04De_ad_new.data<-data.frame(Drought_04De_ad_new2.data$PAR_anomaly,Drought_04De_ad_new2.data$VPD_anomaly,+
...
names(Drought_04De_ad_new.data)<-c('PAR_anomaly','VPD_anomaly','Pre_anomaly','MCWD_anomaly',  
                                   'SoilFertility','SoilClay_content','HAND',   
                                   'TreeHeight','Drought_Length','MCWD_STD', 'WTD',
                                   'EVI_anomaly','year','SegGeo_number','HAND_CLASS')
summary(Drought_04De_ad_new.data)

Drought_04De_ad_new.data<-Drought_04De_ad_new.data[which(Drought_04De_ad_new.data$HAND >=0 & Drought_04De_ad_new.data$HAND <=63 &    Drought_04De_ad_new.data$HAND_CLASS >=-0 ),] #

#----------------------------------------------------------
Drought_04De_ad_new.data$SegGeo_numberf<-factor(Drought_04De_ad_new.data$SegGeo_number)
Drought_04De_ad_new.data2<-Drought_04De_ad_new.data  #before scaling
Drought_04De_ad_new.data_before_scaling<-Drought_04De_ad_new.data 

#--Scale the data---------------------------
Scaled.Drought_04De_ad_new.data<-apply(Drought_04De_ad_new.data[,1:10],2, scale)
Drought_04De_ad_new.data <- as.data.frame( cbind(Scaled.Drought_04De_ad_new.data,Drought_04De_ad_new.data[,11:15]))
...
Ancova_model.lm.slope_model_climate=ancova_establish_slope_Guiana(Drought_04De_ad_new.data)
#Drought_04De_ad_new.data$Prediction=fitted(Ancova_model.lm.slope_model_climate)
...
cbPalette <- c( "#D53E4F","#3288BD", "#238B45")# "#3288BD", "#D53E4F", "#238B45"
...
##------------------Panel A Predictions------ EVI Anomaly as function of WTD---------------------
...
#newd2$WTD_reverse=log2(newd2$WTD_reverse^3)
Hand_class_threshold=40
...
lm.HD_Line_2005_new <-(lm(fit ~ WTD_reverse,data=newd0))
lm.GAM_Line_2005_new<-coef(lm.HD_Line_2005_new)
lm.HD_Line_2010_new <-(lm(fit ~ WTD_reverse,data=newd1))
lm.GAM_Line_2010_new<-coef(lm.HD_Line_2010_new)
lm.HD_Line_2015_new <-(lm(fit ~ WTD_reverse,data=newd2))
lm.GAM_Line_2015_new<-coef(lm.HD_Line_2015_new)
...
##---------------------Figure 3 Panel B-----------------------------------------------------
WTD_set_array=(0:6)*7
cbPalette <- c("#9E0142", "#D53E4F", "#F46D43", "#FDAE61", "#FEE08B", "#E6F598")
...
Figure3_PanelB<-ggplot(tgca_combine, aes(x = PAR_anomaly_reverse, y =fit, colour = factor(type),group=factor(type))) + 
  ...
  # scale_fill_manual(values=rev(paletteer_d("RColorBrewer::YlGnBu")[2:8]))  +    #rev(brewer.pal(8,"rainbow"))
  #   scale_colour_manual(values=rev(paletteer_d("RColorBrewer::YlGnBu")[2:8]))    +theme_few() %+replace%
  scale_fill_manual(values=cbPalette)  +   
...
#------functions for get mean/median values-----------------------
...
get_HAND_class<-function(Input_model.data,class_upbound,interval,size_evi){
  Input_model.data$WTD_Class = 0 #
  min_HAND=0#min(HAND_data_sub)
  for(i in 1:(class_upbound/interval)) {
    
    Input_model.data[which(Input_model.data$Residual_WTD >= (min_HAND+i*interval*1.0) &
                             Input_model.data$Residual_WTD < (min_HAND+(i+1)*1.0*interval) ),]$WTD_Class =(1+i)*interval*1.0+min_HAND
    if (i==0) {
      Input_model.data[which(Input_model.data$Residual_WTD < (min_HAND+(i+1)*1.0*interval) ),] =(1+i)*interval*1.0+min_HAND
    } 
  }     
  return(Input_model.data$WTD_Class)
}
