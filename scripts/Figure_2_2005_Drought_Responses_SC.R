...
BY_increase=100000#
group_number=14
cbPalette1 <- c( "#FEE08B" ,"#FDAE61","#F46D43") #"#D53E4F",  "#F46D43", "#FDAE61", "#FEE08B", 
# choose either 'Modest Drought', 'Medium Drought', 'Severe Drought'
drought_condition_arr<-c('Modest Drought', 'Medium Drought', 'Severe Drought')
for(i in seq(from=1, 3, by=1))
{
  Fig2_PanelCDist<-Draw_Fig2_PanelCDistribution(Drought_ad.data_2005,DroughtSeverity = drought_condition_arr[i],
                                                      x0.var = 'PAR_anomaly',x1.var = 'Drought_Condition',
                                                      rangeMin,rangeMax,BY_increase,group_number,cbPalette1[i])

  print(Fig2_PanelCDist)
  #dev.off()
