...
# These are the remote sensed 1km data; see documentation for definition of variables 
## Read HAND_Ano data files
file_path="F:/csl/HAND/Code_Sum/R/Biogeography_of_Amazon_forests/data/"
Drought_year='2005'                                                                            
file_name=paste('BrandoNIRvCombine_NormalPAR2_wtdhandFULL3_GeoSegSteege_correctLocalMean9_50mRearrange_flood_droughtduration2_dsl2',Drought_year,'.csv',sep='')#_dsl

file_ful_path=paste(file_path,Drought_year,"/",file_name,sep='')
file_ful_path

Drought.data_2005<- read.csv(file=file_ful_path,header=T) 
Drought.data_2005<-Drought.data_2005[which(Drought.data_2005$SegGeo_number >=4),]
...
Drought_ad.data_2005$HAND_CLASS <-Drought_ad.data_2005$HAND_HAND_Class
...
Brando_summary_Geo=summary_group_severity(Drought_ad.data_2005,0) #calculate for original EVI
Brando_summary_Geo$type=rep("Ori", times=length(Brando_summary_Geo$EVI_anomaly))
arr_ori=data.frame(Brando_summary_Geo$HAND_CLASS,Brando_summary_Geo$EVI_anomaly,Brando_summary_Geo$ci,Brando_summary_Geo$type, Brando_summary_Geo$Drought_Condition)

tgca_combine<-rbind(arr_ori)#
tgca_combine$Drought_severity=""
tgca_combine[which((tgca_combine$Brando_summary_Geo.Drought_Condition =="Modest Drought") |  (tgca_combine$Brando_summary_Geo.Drought_Condition =="Lightest Drought") & (tgca_combine$Brando_summary_Geo.type == "Ori") ),]$Drought_severity="Meodest_Ori"
tgca_combine[which((tgca_combine$Brando_summary_Geo.Drought_Condition =="Medium Drought") & (tgca_combine$Brando_summary_Geo.type == "Ori") & tgca_combine$Brando_summary_Geo.HAND_CLASS >=-1),]$Drought_severity="Moedium_Ori"
tgca_combine[which((tgca_combine$Brando_summary_Geo.Drought_Condition  =="Severe Drought") & (tgca_combine$Brando_summary_Geo.type == "Ori") & tgca_combine$Brando_summary_Geo.HAND_CLASS >=-1),]$Drought_severity="Severe_Ori"


rangeMin=-0.5
rangeMax=rangeMin+1
BY_increase=0.5
Figure2_PanleB<- ggplot_adjustment_withSeverity(tgca_combine,rangeMin,rangeMax,BY_increase,Drought_year,SWTD_MeanEVI,DWTD_MeanEVI) 
Figure2_PanleB
#--------------------------------------------------------------------------------------------

#--------------Figure 2 Pannel C-------------------------------------------------------------
## 1. General EVI Anomaly VS Par class  with shallow to deep water tables--------------------
Drought_ad.data_2005<-data.frame(Drought.data_2005)
Drought_ad.data_2005<-Drought_ad.data_2005[which(Drought_ad.data_2005$WTD<=500),]

Drought_ad.data_2005<-Drought_ad.data_2005[which(is.finite(Drought_ad.data_2005$EVI_anomaly) & 
                                        is.finite(Drought_ad.data_2005$PAR_anomaly) ),]


#-----a way of group for par -------------------
...
#--------------Figure 2 Pannel C Distribution---------------------------------------------------
##-------------------PAR distribution----1km  drought severity----------------------------------
library(gtable)
library(grid)
Drought_ad.data_2005<-data.frame(Drought.data_2005)
...
Fig2_PanelCDistribution<-Draw_Fig2_PanelCDistribution(Drought_ad.data_2005,DroughtSeverity = 'Modest Drought',x0.var = 'PAR_anomaly',x1.var = 'Drought_Condition',rangeMin,rangeMax,BY_increase,group_number,cbPalette1)
...
ggplot_adjustment_withSeverity<-function(tgca,rangeMin,rangeMax,BY_increase,Drought_year,SWTD_MeanEVI,DWTD_MeanEVI){
  ...
  ggplot(tgca, aes(x=Brando_summary_Geo.HAND_CLASS, y=Brando_summary_Geo.EVI_anomaly, colour = factor(Drought_severity),group=factor(Drought_severity)))+ #, colour=Drought_Condition, group=Drought_Condition))+ , group=tgc1.type tgc1.Droughtlength
    geom_errorbar(aes(ymin=Brando_summary_Geo.EVI_anomaly-Brando_summary_Geo.ci, ymax=Brando_summary_Geo.EVI_anomaly+Brando_summary_Geo.ci,colour = factor(Drought_severity),group=factor(Drought_severity)), size=0.5, width=.3,face="bold") +
